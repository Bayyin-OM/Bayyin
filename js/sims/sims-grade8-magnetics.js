...
function simMagnets1() {
  ...
  function gp(e){const r=cv.getBoundingClientRect(),sc=cv.width/r.width,s=e.touches&&e.touches[0]||e.changedTouches&&e.changedTouches[0]||e;return{x:(s.clientX-r.left)*sc,y:(s.clientY-r.top)*sc};}
  ...
  function fieldLines(sources,w,h){
    ...
    sources.forEach(src=>{
      ...
      for(let li=0;li<N;li++){
        ...
        // collect points for arrow drawing
        const pts=[[px,py]];
        for(let s=0;s<180;s++){
          let fx=0,fy=0;
          sources.forEach(s2=>{
        ...
        const col=`rgba(26,143,168,${alpha})`;
        // draw line
        c.beginPath(); c.moveTo(pts[0][0],pts[0][1]);
        for(let i=1;i<pts.length;i++) c.lineTo(pts[i][0],pts[i][1]);
        c.strokeStyle=col; c.lineWidth=1.5; c.stroke();
        // draw arrow at ~40% and ~75% of the line
        if(pts.length>8){
          const alphaArrow=0.25+(li%5===0?0.45:li%3===0?0.25:0.1);
          const arrowCol=`rgba(26,143,168,${alphaArrow})`;
          [0.4,0.72].forEach(frac=>{
            const idx=Math.floor(frac*pts.length);
            if(idx<2||idx>=pts.length) return;
            const [ax,ay]=pts[idx];
            const [bx,by]=pts[idx-2];
...
// ══════════════════════════════════════════════════════════════
function _drawCompassMF2(c,cx,cy,R,angle){
  c.save(); c.shadowColor='rgba(0,0,0,0.12)'; c.shadowBlur=10; c.shadowOffsetY=3;
  c.fillStyle='#fff'; c.strokeStyle='#BDC3C7'; c.lineWidth=1.5;
  c.beginPath(); c.arc(cx,cy,R,0,Math.PI*2); c.fill(); c.stroke(); c.restore();
  for(let i=0;i<12;i++){
    const a=i/12*Math.PI*2,r1=i%3===0?R*.7:R*.82;
    c.strokeStyle=i%3===0?'#95A5A6':'#D5D8DC'; c.lineWidth=i%3===0?1.5:1;
    c.beginPath(); c.moveTo(cx+Math.cos(a)*r1,cy+Math.sin(a)*r1); c.lineTo(cx+Math.cos(a)*R*.94,cy+Math.sin(a)*R*.94); c.stroke();
  }
  c.save(); c.translate(cx,cy); c.rotate(angle);
  c.fillStyle='#E74C3C';
  c.beginPath(); c.moveTo(0,-R*.76); c.lineTo(R*.13,0); c.lineTo(0,R*.15); c.lineTo(-R*.13,0); c.closePath(); c.fill();
  c.fillStyle='#95A5A6';
  c.beginPath(); c.moveTo(0,R*.76); c.lineTo(R*.13,0); c.lineTo(0,-R*.15); c.lineTo(-R*.13,0); c.closePath(); c.fill();
  c.fillStyle='#2C3E50'; c.beginPath(); c.arc(0,0,R*.1,0,Math.PI*2); c.fill();
  c.restore();
  c.font=`bold ${Math.round(R*.3)}px Tajawal`; c.textAlign='center'; c.textBaseline='middle';
  c.save(); c.translate(cx,cy); c.rotate(angle);
  c.fillStyle='#E74C3C'; c.fillText('ش',0,-R*.5);
  c.fillStyle='#6C7A7D'; c.fillText('ج',0,R*.5);
  c.restore();
}
...
function simMagField2() {
  ...
  // State: magnet fixed centre, compass draggable
  if(!simState.mf2) simState.mf2={
    magX:.5, magY:.5, flipped:false,
    compX:.75, compY:.3,          // compass position (normalised)
    needleAngle:0,                 // current rendered angle (animated)
    dragMag:false, dragComp:false,
    omx:0,omy:0, ocx:0,ocy:0
  };
  ...
  controls(`
    <div class="ctrl-section">
      ...
      <button class="ctrl-btn reset" onclick="simState.mf2.compX=.22;simState.mf2.compY=.28">↺ أعِد البوصلة</button>
    </div>
    <div class="info-box" style="font-size:13px;line-height:2">
      🔴 الإبرة الحمراء = القطب <strong>الشمالي</strong> للبوصلة<br>
      تنجذب نحو القطب <strong>الجنوبي (ج)</strong> للمغناطيس<br><br>
      ✋ اسحب البوصلة أو المغناطيس
    </div>
    <div id="mf2Lbl" style="margin-top:10px;font-size:14px;font-weight:600;line-height:1.7;padding:10px 12px;background:rgba(26,143,168,0.08);border-radius:8px;min-height:44px;text-align:center"></div>`);
  ...
  // Field vector at point (px,py) from two poles
  function fieldAt(px,py,nPoleX,nPoleY,sPoleX,sPoleY){
    const dnx=px-nPoleX,dny=py-nPoleY,dn=Math.max(15,Math.hypot(dnx,dny));
    const dsx=px-sPoleX,dsy=py-sPoleY,ds=Math.max(15,Math.hypot(dsx,dsy));
    const k=8000;
    return{
      fx: k*dnx/(dn*dn*dn) - k*dsx/(ds*ds*ds),
      fy: k*dny/(dn*dn*dn) - k*dsy/(ds*ds*ds)
    };
  }
  ...
  function onDown(e){
    ...
    const cx2=S.compX*w,cy2=S.compY*h;
    if(Math.hypot(p.x-cx2,p.y-cy2)<40){
      S.dragComp=true; S.ocx=p.x-cx2; S.ocy=p.y-cy2;
    } else if(Math.hypot(p.x-mx,p.y-my)<60){
      S.dragMag=true; S.omx=p.x-mx; S.omy=p.y-my;
    }
  }
  function onMove(e){
    const p=gp(e),w=cv.width,h=cv.height;
    if(S.dragComp){
      S.compX=Math.max(.04,Math.min(.96,(p.x-S.ocx)/w));
      S.compY=Math.max(.06,Math.min(.9,(p.y-S.ocy)/h));
    } else if(S.dragMag){
      S.magX=Math.max(.1,Math.min(.9,(p.x-S.omx)/w));
      S.magY=Math.max(.1,Math.min(.85,(p.y-S.omy)/h));
    }
  }
  function onUp(){ S.dragComp=false; S.dragMag=false; }
  ...
  function drawCompass(cx2,cy2,R,angle){
    // shadow
    c.save();
    c.shadowColor='rgba(0,0,0,0.18)'; c.shadowBlur=10; c.shadowOffsetY=3;
    c.fillStyle='#FAFAFA'; c.strokeStyle='#BDC3C7'; c.lineWidth=2;
    c.beginPath(); c.arc(cx2,cy2,R,0,Math.PI*2); c.fill(); c.stroke();
    c.restore();
    // tick marks
    for(let i=0;i<12;i++){
      const a=i/12*Math.PI*2;
      const r1=i%3===0?R*.72:R*.82;
      c.strokeStyle=i%3===0?'#95A5A6':'#BDC3C7'; c.lineWidth=i%3===0?2:1;
      c.beginPath();
      c.moveTo(cx2+Math.cos(a)*r1,cy2+Math.sin(a)*r1);
      c.lineTo(cx2+Math.cos(a)*R*.92,cy2+Math.sin(a)*R*.92);
      c.stroke();
    }
    // needle
    ...
    // centre dot
    c.fillStyle='#2C3E50';
    c.beginPath(); c.arc(0,0,R*.1,0,Math.PI*2); c.fill();
    c.restore();
    // N label
    c.save(); c.translate(cx2,cy2); c.rotate(angle);
    c.font=`bold ${Math.round(R*.28)}px Tajawal`;
  ...
  // draw faint field-line background (self-contained, no external dependency)
  function drawBgLines(nPoleX,nPoleY,sPoleX,sPoleY,w,h){
    const sources=[{cx:nPoleX,cy:nPoleY,sign:1},{cx:sPoleX,cy:sPoleY,sign:-1}];
    sources.forEach(src=>{
      if(src.sign<0) return;
      const N=20;
      for(let li=0;li<N;li++){
        ...
        for(let s=0;s<180;s++){
          let fx=0,fy=0;
          sources.forEach(s2=>{
            const dx=px-s2.cx,dy=py-s2.cy,d=Math.max(18,Math.hypot(dx,dy));
            fx+=s2.sign*dx/(d*d*d)*9000; fy+=s2.sign*dy/(d*d*d)*9000;
          });
          const fm=Math.hypot(fx,fy)||1;
          px+=fx/fm*4.5; py+=fy/fm*4.5;
          let hitS=false;
        ...
        const alpha=0.10+(li%5===0?0.18:li%3===0?0.08:0.03);
        c.beginPath(); c.moveTo(pts[0][0],pts[0][1]);
        for(let i=1;i<pts.length;i++) c.lineTo(pts[i][0],pts[i][1]);
        c.strokeStyle=`rgba(26,143,168,${alpha})`; c.lineWidth=1.5; c.stroke();
        // arrows
        if(pts.length>8){
          const arrowAlpha=0.22+(li%5===0?0.4:li%3===0?0.2:0.08);
          [0.4,0.72].forEach(frac=>{
            const idx=Math.floor(frac*pts.length);
            if(idx<2||idx>=pts.length) return;
            const [ax,ay]=pts[idx],[bx,by]=pts[idx-2];
            const angle=Math.atan2(ay-by,ax-bx),sz=5;
            c.save(); c.translate(ax,ay); c.rotate(angle);
            c.beginPath(); c.moveTo(sz,0); c.lineTo(-sz,-sz*.55); c.lineTo(-sz*.3,0); c.lineTo(-sz,sz*.55); c.closePath();
            c.fillStyle=`rgba(26,143,168,${arrowAlpha})`; c.fill();
            c.restore();
          });
        }
      }
  ...
  function draw(){
    ...
    const w=cv.width,h=cv.height;
    c.clearRect(0,0,w,h);
    c.fillStyle='#F7F8FC'; c.fillRect(0,0,w,h);

    const mx=S.magX*w, my=S.magY*h;
    const mW=Math.min(w*.24,140), mH=Math.min(h*.14,50);
    const nPoleX=S.flipped?mx-mW*.26:mx+mW*.26;
    const sPoleX=S.flipped?mx+mW*.26:mx-mW*.26;
    const cx2=S.compX*w, cy2=S.compY*h;
    const R=Math.min(w*.082,56);

    // compute field & smooth rotation
    const {fx,fy}=fieldAt(cx2,cy2,nPoleX,my,sPoleX,my);
    const targetAngle=Math.atan2(fy,fx);
    let diff=targetAngle-S.needleAngle;
    while(diff>Math.PI)diff-=Math.PI*2; while(diff<-Math.PI)diff+=Math.PI*2;
    S.needleAngle+=diff*0.10;

    const dToS=Math.hypot(cx2-sPoleX,cy2-my);
    const dToN=Math.hypot(cx2-nPoleX,cy2-my);
    const isNearS=dToS<dToN;

    // field lines (8 lines, self-contained)
    const sources=[{cx:nPoleX,cy:my,sign:1},{cx:sPoleX,cy:my,sign:-1}];
    sources.forEach(src=>{
      if(src.sign<0)return;
      for(let li=0;li<8;li++){
        const a0=(li/8)*Math.PI*2;
        let px=src.cx+Math.cos(a0)*18, py=src.cy+Math.sin(a0)*18;
        const pts=[[px,py]];
        for(let s=0;s<160;s++){
          let ffx=0,ffy=0;
          sources.forEach(s2=>{const ddx=px-s2.cx,ddy=py-s2.cy,dd=Math.max(15,Math.hypot(ddx,ddy));ffx+=s2.sign*ddx/(dd**3)*8000;ffy+=s2.sign*ddy/(dd**3)*8000;});
          const fm=Math.hypot(ffx,ffy)||1; px+=ffx/fm*5; py+=ffy/fm*5;
          let hit=false; sources.forEach(s2=>{if(s2.sign<0&&Math.hypot(px-s2.cx,py-s2.cy)<18)hit=true;});
          if(hit||px<1||px>w-1||py<1||py>h-1)break;
          pts.push([px,py]);
        }
        c.beginPath(); c.moveTo(pts[0][0],pts[0][1]);
        for(let i=1;i<pts.length;i++)c.lineTo(pts[i][0],pts[i][1]);
        c.strokeStyle='rgba(52,152,219,0.22)'; c.lineWidth=1.5; c.stroke();
        if(pts.length>6){
          const idx=Math.floor(pts.length*.55);
          if(idx>=2){
            const [ax,ay]=pts[idx],[bx,by]=pts[idx-2],ang=Math.atan2(ay-by,ax-bx),sz=6;
            c.save(); c.translate(ax,ay); c.rotate(ang);
            c.fillStyle='rgba(52,152,219,0.4)';
            c.beginPath(); c.moveTo(sz,0); c.lineTo(-sz,-sz*.5); c.lineTo(-sz*.3,0); c.lineTo(-sz,sz*.5); c.closePath(); c.fill();
            c.restore();
          }
        }
      }
    ...
    // attraction arrow (red dashed)
    const aPX=isNearS?sPoleX:nPoleX;
    const daa=Math.hypot(cx2-aPX,cy2-my);
    if(daa>R+mW*.3){
      const ddx=(aPX-cx2)/daa, ddy=(my-cy2)/daa;
      const fx2=cx2+ddx*(R+4), fy2=cy2+ddy*(R+4);
      const tx=aPX-ddx*mW*.28, ty=my-ddy*mW*.28;
      c.save(); c.setLineDash([5,4]);
      c.strokeStyle='rgba(231,76,60,0.65)'; c.lineWidth=2.5;
      c.beginPath(); c.moveTo(fx2,fy2); c.lineTo(tx,ty); c.stroke();
      const aAng=Math.atan2(ty-fy2,tx-fx2);
      c.setLineDash([]); c.fillStyle='rgba(231,76,60,0.85)';
      c.beginPath(); c.moveTo(tx,ty);
      c.lineTo(tx-12*Math.cos(aAng-.4),ty-12*Math.sin(aAng-.4));
      c.lineTo(tx-12*Math.cos(aAng+.4),ty-12*Math.sin(aAng+.4));
      c.closePath(); c.fill(); c.restore();
    }

    _drawBarMagnet(c,mx,my,mW,mH,S.flipped);
    _drawCompassMF2(c,cx2,cy2,R,S.needleAngle);

    // update sidebar label
    const lbl=document.getElementById('mf2Lbl');
    if(lbl){
      lbl.innerHTML=isNearS
        ?'🔴 الإبرة الحمراء <strong>(ش)</strong> تنجذب نحو القطب <strong style="color:#C0392B">الجنوبي (ج)</strong>'
        :'🔵 الإبرة الحمراء <strong>(ش)</strong> تبتعد عن القطب <strong style="color:#2980B9">الشمالي (ش)</strong>';
    }
...
// ══════════════════════════════════════════════════════════════
// 11-3 · TAB 3 — المجال المغناطيسي الأرضي
// ══════════════════════════════════════════════════════════════
function simMagField3() {
  cancelAnimationFrame(animFrame);
  const cv=document.getElementById('simCanvas');
  const c=cv.getContext('2d');

  if(!simState.mf3) simState.mf3={
    magnetX:0.84, magnetY:0.38,
    flipped:false, drag:false, ox:0, oy:0, t:0
  };
  const S=simState.mf3;

  controls(`
    <div class="ctrl-section">
      <div class="ctrl-label">🌍 المجال المغناطيسي الأرضي</div>
      <div style="font-size:13px;color:#555;line-height:1.8">
        اسحب المغناطيس نحو أي قطب<br>
        جغرافي وشاهد التجاذب أو التنافر
      </div>
      <button class="ctrl-btn" onclick="simState.mf3.flipped=!simState.mf3.flipped">🔄 اقلب المغناطيس</button>
      <button class="ctrl-btn reset" onclick="simState.mf3.magnetX=0.84;simState.mf3.magnetY=0.38;simState.mf3.flipped=false">↺ إعادة</button>
    </div>
    <div class="info-box" style="font-size:13px;line-height:2">
      🌍 القطب الشمالي الجغرافي<br>
      &nbsp;&nbsp;← مغناطيسياً: <strong style="color:#2980B9">جنوبي (ج)</strong><br>
      &nbsp;&nbsp;← يجذب قطب الم. <strong>الشمالي</strong> ❤️<br><br>
      🌍 القطب الجنوبي الجغرافي<br>
      &nbsp;&nbsp;← مغناطيسياً: <strong style="color:#C0392B">شمالي (ش)</strong><br>
      &nbsp;&nbsp;← يجذب قطب الم. <strong>الجنوبي</strong> ❤️
    </div>`);

  function gp(e){const r=cv.getBoundingClientRect(),sc=cv.width/r.width,s=e.touches&&e.touches[0]||e.changedTouches&&e.changedTouches[0]||e;return{x:(s.clientX-r.left)*sc,y:(s.clientY-r.top)*sc};}
  cv.addEventListener('mousedown',e=>{const p=gp(e),w=cv.width,h=cv.height;const mx=S.magnetX*w,my=S.magnetY*h;if(Math.hypot(p.x-mx,p.y-my)<65){S.drag=true;S.ox=p.x-mx;S.oy=p.y-my;}});
  cv.addEventListener('mousemove',e=>{if(!S.drag)return;const p=gp(e),w=cv.width,h=cv.height;S.magnetX=Math.max(.62,Math.min(.97,(p.x-S.ox)/w));S.magnetY=Math.max(.05,Math.min(.9,(p.y-S.oy)/h));});
  cv.addEventListener('mouseup',()=>S.drag=false);
  cv.addEventListener('touchstart',e=>{e.preventDefault();const p=gp(e),w=cv.width,h=cv.height;const mx=S.magnetX*w,my=S.magnetY*h;if(Math.hypot(p.x-mx,p.y-my)<65){S.drag=true;S.ox=p.x-mx;S.oy=p.y-my;}},{passive:false});
  cv.addEventListener('touchmove',e=>{e.preventDefault();if(!S.drag)return;const p=gp(e),w=cv.width,h=cv.height;S.magnetX=Math.max(.62,Math.min(.97,(p.x-S.ox)/w));S.magnetY=Math.max(.05,Math.min(.9,(p.y-S.oy)/h));},{passive:false});
  cv.addEventListener('touchend',()=>S.drag=false);

  function drawEarth(cx,cy,r){
    // Atmosphere glow
    const atm=c.createRadialGradient(cx,cy,r*0.92,cx,cy,r*1.18);
    atm.addColorStop(0,'rgba(100,180,255,0.18)'); atm.addColorStop(1,'transparent');
    c.fillStyle=atm; c.beginPath(); c.arc(cx,cy,r*1.18,0,Math.PI*2); c.fill();

    // Ocean
    const eg=c.createRadialGradient(cx-r*.28,cy-r*.28,r*.04,cx,cy,r);
    eg.addColorStop(0,'#6EC6F0'); eg.addColorStop(0.45,'#2E86C1'); eg.addColorStop(1,'#1A4F7A');
    c.fillStyle=eg; c.beginPath(); c.arc(cx,cy,r,0,Math.PI*2); c.fill();
    ...
    // Africa
    c.fillStyle='#3DBE6A';
    c.beginPath(); c.moveTo(cx-r*.08,cy-r*.38);
    c.bezierCurveTo(cx+r*.18,cy-r*.42, cx+r*.22,cy-r*.18, cx+r*.16,cy+r*.02);
    c.bezierCurveTo(cx+r*.12,cy+r*.18, cx+r*.02,cy+r*.42, cx-r*.04,cy+r*.48);
    c.bezierCurveTo(cx-r*.16,cy+r*.42, cx-r*.22,cy+r*.18, cx-r*.18,cy+r*.02);
    c.bezierCurveTo(cx-r*.2,cy-r*.1, cx-r*.18,cy-r*.3, cx-r*.08,cy-r*.38);
    c.fill();

    // Americas
    c.fillStyle='#2EAA5C';
    c.beginPath(); c.moveTo(cx-r*.55,cy-r*.45);
    c.bezierCurveTo(cx-r*.38,cy-r*.5, cx-r*.32,cy-r*.3, cx-r*.34,cy-r*.1);
    c.bezierCurveTo(cx-r*.32,cy+r*.1, cx-r*.38,cy+r*.3, cx-r*.44,cy+r*.5);
    c.bezierCurveTo(cx-r*.56,cy+r*.44, cx-r*.62,cy+r*.22, cx-r*.6,cy+r*.05);
    c.bezierCurveTo(cx-r*.58,cy-r*.15, cx-r*.62,cy-r*.35, cx-r*.55,cy-r*.45);
    c.fill();

    // Asia
    c.fillStyle='#38C26E';
    c.beginPath(); c.moveTo(cx+r*.18,cy-r*.42);
    c.bezierCurveTo(cx+r*.45,cy-r*.48, cx+r*.68,cy-r*.32, cx+r*.72,cy-r*.12);
    c.bezierCurveTo(cx+r*.7,cy+r*.05, cx+r*.55,cy+r*.18, cx+r*.38,cy+r*.12);
    c.bezierCurveTo(cx+r*.28,cy+r*.08, cx+r*.18,cy-r*.05, cx+r*.18,cy-r*.42);
    c.fill();

    // Australia
    c.fillStyle='#2EAA5C';
    c.beginPath(); c.ellipse(cx+r*.55,cy+r*.35,r*.16,r*.13,0.2,0,Math.PI*2); c.fill();

    // Antarctica
    c.fillStyle='rgba(240,248,255,0.82)';
    c.beginPath(); c.ellipse(cx,cy+r,r*0.72,r*0.18,0,0,Math.PI*2); c.fill();

    // Arctic
    c.fillStyle='rgba(240,248,255,0.72)';
    c.beginPath(); c.ellipse(cx,cy-r,r*0.6,r*0.15,0,0,Math.PI*2); c.fill();

    c.restore();

    // Globe rim shading
    const rim=c.createRadialGradient(cx-r*.4,cy-r*.4,r*.2,cx,cy,r);
    rim.addColorStop(0,'rgba(255,255,255,0.12)'); rim.addColorStop(0.7,'transparent'); rim.addColorStop(1,'rgba(0,30,80,0.28)');
    c.fillStyle=rim; c.beginPath(); c.arc(cx,cy,r,0,Math.PI*2); c.fill();
    c.strokeStyle='rgba(100,180,255,0.35)'; c.lineWidth=2; c.beginPath(); c.arc(cx,cy,r,0,Math.PI*2); c.stroke();

    // Geographic pole labels outside globe
    const fs=Math.max(12,Math.round(r*.19));
    c.font=`bold ${fs}px Tajawal`; c.textAlign='center'; c.textBaseline='middle';
    c.fillStyle='rgba(200,230,255,0.95)'; c.fillText('◉ ش. جغرافي',cx,cy-r-r*.22);
    c.fillStyle='rgba(200,230,255,0.95)'; c.fillText('◉ ج. جغرافي',cx,cy+r+r*.22);

    // Magnetic pole dots on surface — NO text inside globe
    const axR=r*.62;
    c.fillStyle='rgba(41,128,185,0.9)';
    c.beginPath(); c.arc(cx,cy-axR,r*.09,0,Math.PI*2); c.fill();
    c.strokeStyle='rgba(150,210,255,0.9)'; c.lineWidth=2; c.stroke();

    c.fillStyle='rgba(192,57,43,0.9)';
    c.beginPath(); c.arc(cx,cy+axR,r*.09,0,Math.PI*2); c.fill();
    c.strokeStyle='rgba(255,150,130,0.9)'; c.lineWidth=2; c.stroke();

    // Legend labels beside dots
    const lfs=Math.max(10,Math.round(r*.15));
    c.font=`bold ${lfs}px Tajawal`; c.textBaseline='middle';
    c.fillStyle='rgba(100,180,255,0.9)'; c.textAlign='left'; c.fillText(' مغناطيسي ج',cx+r*.12,cy-axR);
    c.fillStyle='rgba(255,120,100,0.9)'; c.fillText(' مغناطيسي ش',cx+r*.12,cy+axR);
  }

  function drawFieldLines3(earthCx,earthCy,earthR){
    const sPoleX=earthCx, sPoleY=earthCy-earthR*.6; // geographic N = magnetic S
    const nPoleX=earthCx, nPoleY=earthCy+earthR*.6; // geographic S = magnetic N
    // Only draw 6 distinct field lines at clearly separated angles
    const lineAngles=[Math.PI*0.08, Math.PI*0.2, Math.PI*0.38, Math.PI*0.62, Math.PI*0.8, Math.PI*0.92];
    lineAngles.forEach((a0,li)=>{
      let px=nPoleX+Math.cos(a0)*earthR*.22, py=nPoleY+Math.sin(a0)*earthR*.22;
      const pts=[[px,py]];
      for(let s=0;s<300;s++){
        const dxS=px-sPoleX,dyS=py-sPoleY,rS=Math.max(18,Math.hypot(dxS,dyS));
        const dxN=px-nPoleX,dyN=py-nPoleY,rN=Math.max(18,Math.hypot(dxN,dyN));
        let fx=(-1)*dxS/(rS*rS*rS)*9000+(1)*dxN/(rN*rN*rN)*9000;
        let fy=(-1)*dyS/(rS*rS*rS)*9000+(1)*dyN/(rN*rN*rN)*9000;
        const fm=Math.hypot(fx,fy)||1;
        px+=fx/fm*5; py+=fy/fm*5;
        if(Math.hypot(px-earthCx,py-earthCy)<earthR*.88) break;
        const w2=cv.width,h2=cv.height;
        if(px<-30||px>w2+30||py<-30||py>h2+30) break;
        pts.push([px,py]);
      }
      // Draw line with distinct color per line group
      const colors=['rgba(80,160,255,0.7)','rgba(100,200,255,0.65)','rgba(140,220,255,0.55)','rgba(140,220,255,0.55)','rgba(100,200,255,0.65)','rgba(80,160,255,0.7)'];
      c.beginPath(); c.moveTo(pts[0][0],pts[0][1]);
      pts.forEach(p=>c.lineTo(p[0],p[1]));
      c.strokeStyle=colors[li]; c.lineWidth=2.2; c.stroke();

      // Arrow showing direction (N→S outside Earth)
      if(pts.length>20){
        const mid=Math.floor(pts.length*0.5);
        const [ax,ay]=pts[mid]; const [bx,by]=pts[Math.min(mid+3,pts.length-1)];
        const angle=Math.atan2(by-ay,bx-ax);
        c.save(); c.translate(ax,ay); c.rotate(angle);
        c.fillStyle=colors[li]; c.beginPath(); c.moveTo(8,0); c.lineTo(-5,-5); c.lineTo(-5,5); c.closePath(); c.fill();
        c.restore();
      }
    });
  }

  function drawMagnet(mx,my,mW,mH){
    // S on left (ج), N on right (ش) — or flipped
    const nCol='#C0392B', sCol='#2980B9';
    const leftCol=S.flipped?nCol:sCol;
    const rightCol=S.flipped?sCol:nCol;
    const leftLbl=S.flipped?'ش':'ج';
    const rightLbl=S.flipped?'ج':'ش';
    const grad=c.createLinearGradient(mx-mW/2,my,mx+mW/2,my);
    grad.addColorStop(0,leftCol); grad.addColorStop(.48,'rgba(255,255,255,.1)'); grad.addColorStop(1,rightCol);
    c.fillStyle=grad; c.beginPath(); c.roundRect(mx-mW/2,my-mH/2,mW,mH,7); c.fill();
    c.strokeStyle='rgba(0,0,0,.35)'; c.lineWidth=2.5; c.beginPath(); c.roundRect(mx-mW/2,my-mH/2,mW,mH,7); c.stroke();
    const fs2=Math.round(mH*.5);
    c.font=`bold ${fs2}px Tajawal`; c.textAlign='center'; c.textBaseline='middle'; c.fillStyle='white';
    c.fillText(leftLbl,mx-mW*.28,my); c.fillText(rightLbl,mx+mW*.28,my);
  }

  function draw(){
    if(currentSim!=='magfield'||currentTab!==2)return;
    S.t+=.02;
    const w=cv.width,h=cv.height;
    c.clearRect(0,0,w,h);
    ...
    bg.addColorStop(0,'#080C1A'); bg.addColorStop(1,'#111830');
    c.fillStyle=bg; c.fillRect(0,0,w,h);

    // Stars
    for(let i=0;i<60;i++){
      const sx=i*137.5%w, sy=i*93.1%h;
      const sr=.4+Math.sin(S.t*1.2+i)*.35;
      c.fillStyle=`rgba(255,255,255,${.3+Math.sin(S.t+i*.7)*.2})`; c.beginPath(); c.arc(sx,sy,sr,0,Math.PI*2); c.fill();
    }

    const earthR=Math.min(w*.22,h*.30,110);
    const earthCx=w*.38, earthCy=h*.5;

    drawFieldLines3(earthCx,earthCy,earthR);
    drawEarth(earthCx,earthCy,earthR);

    // Magnet
    const mW=Math.min(w*.24,115), mH=Math.min(h*.09,40);
    const mx=S.magnetX*w, my=S.magnetY*h;
    // Pole positions
    const magLeftX=mx-mW*.28, magLeftY=my;   // ج (S) when not flipped
    const magRightX=mx+mW*.28, magRightY=my; // ش (N) when not flipped
    const magNX=S.flipped?magLeftX:magRightX;
    const magNY=my;
    const magSX=S.flipped?magRightX:magLeftX;
    const magSY=my;

    // Earth pole positions
    const earthGeogNX=earthCx, earthGeogNY=earthCy-earthR; // geographic N (magnetic S)
    const earthGeogSX=earthCx, earthGeogSY=earthCy+earthR; // geographic S (magnetic N)
    const earthMagNX=earthCx, earthMagNY=earthCy+earthR*.58;
    const earthMagSX=earthCx, earthMagSY=earthCy-earthR*.58;

    // Find which pair is closest
    const pairs=[
      {fromX:magNX,fromY:magNY,toX:earthMagSX,toY:earthMagSY,attract:true,  label:'ش(م.) ↔ ج(م.) ❤️ تجاذب'},
      {fromX:magNX,fromY:magNY,toX:earthMagNX,toY:earthMagNY,attract:false, label:'ش(م.) ↔ ش(م.) 💥 تنافر'},
      {fromX:magSX,fromY:magSY,toX:earthMagNX,toY:earthMagNY,attract:true,  label:'ج(م.) ↔ ش(م.) ❤️ تجاذب'},
      {fromX:magSX,fromY:magSY,toX:earthMagSX,toY:earthMagSY,attract:false, label:'ج(م.) ↔ ج(م.) 💥 تنافر'},
    ];

    const threshold=w*.35;
    pairs.forEach(p=>{
      const dist=Math.hypot(p.fromX-p.toX,p.fromY-p.toY);
      const prox=Math.max(0,1-dist/threshold);
      if(prox<.08) return;
      const col=p.attract?'rgba(39,174,96,':'rgba(231,76,60,';
      c.strokeStyle=col+(prox*.7)+')'; c.lineWidth=prox*4+1;
      c.setLineDash([5,5]); c.beginPath(); c.moveTo(p.fromX,p.fromY); c.lineTo(p.toX,p.toY); c.stroke(); c.setLineDash([]);
      if(prox>.28){
        c.font=`bold ${11+prox*5|0}px Tajawal`; c.textAlign='center'; c.textBaseline='middle';
        c.fillStyle=p.attract?'#4AE88A':'#FF6060';
        c.fillText(p.label,(p.fromX+p.toX)/2,(p.fromY+p.toY)/2-(dist*.12+18));
      }
    });

    drawMagnet(mx,my,mW,mH);
    if(!S.drag && S.t<10){
      c.font='12px Tajawal'; c.fillStyle='rgba(180,200,255,0.55)'; c.textAlign='center'; c.textBaseline='middle';
      c.fillText('✋ اسحب المغناطيس نحو الأرض',mx,my+mH+20);
    }

    // Title
    c.font=`bold ${Math.round(h*.038)}px Tajawal`; c.fillStyle='rgba(180,210,255,0.8)'; c.textAlign='center'; c.textBaseline='top';
    c.fillText('الأرض كمغناطيس ضخم',w/2,h*.02);
...
function simStaticElec1() {
  ...
  const OBJECTS = [
    { id:'thread',  emoji:'🧵', name:'خيط',     attracted:true  },
    { id:'paper',   emoji:'📄', name:'ورق',     attracted:true  },
    { id:'plastic', emoji:'🧴', name:'بلاستيك', attracted:true  },
    { id:'gold',    emoji:'🥇', name:'ذهب',     attracted:false },
    { id:'copper',  emoji:'🔌', name:'نحاس',    attracted:false },
    { id:'silver',  emoji:'🥈', name:'فضة',     attracted:false },
  ];

  if (!simState.se1) simState.se1 = {
    charge: 0, t: 0,
    balloonX: .22, balloonY: .38,
    drag: false, ox: 0, oy: 0,
    rubCount: 0, lastRubY: 0,
    // per-object test results  { id → 'attracted'|'repelled'|null }
    results: {},
    // active test object being approached
    activeObj: null,
  };
  const S = simState.se1;

  function buildControls() {
    const charged = S.charge >= 3;
    // Build results summary
    let resHTML = '';
    OBJECTS.forEach(obj => {
      const r = S.results[obj.id];
      if (r === 'attracted')  resHTML += `
        <div style="display:flex;align-items:center;gap:7px;padding:5px 8px;background:rgba(39,174,96,0.12);border:1px solid rgba(39,174,96,0.35);border-radius:8px;margin-bottom:5px">
          <span style="font-size:18px">${obj.emoji}</span>
          <div style="flex:1">
            <div style="font-weight:800;color:#1E8449;font-size:12px">${obj.name}</div>
            <div style="color:#27AE60;font-size:11px">✅ يُجذب نحو البالون</div>
          </div>
        </div>`;
      if (r === 'repelled')   resHTML += `
        <div style="display:flex;align-items:center;gap:7px;padding:5px 8px;background:rgba(192,57,43,0.10);border:1px solid rgba(192,57,43,0.30);border-radius:8px;margin-bottom:5px">
          <span style="font-size:18px">${obj.emoji}</span>
          <div style="flex:1">
            <div style="font-weight:800;color:#922B21;font-size:12px">${obj.name}</div>
            <div style="color:#C0392B;font-size:11px">❌ لا يُجذب (فلز)</div>
          </div>
        </div>`;
    });
    ...
    controls(`
      <div class="ctrl-section">
        ...
        <div style="font-size:13px;line-height:1.8;color:#555">
          اسحب البالون على شعر الشخص<br>لشحنه، ثم اختبر كل جسم!
        </div>
        <div id="seCharge" style="text-align:center;font-size:17px;font-weight:800;margin-top:6px;color:#C0392B">
          الشحنة: ${S.charge > 0 ? '➕'.repeat(S.charge) : 'ـ ـ ـ ـ ـ'}
        </div>
      </div>
      ${resHTML ? `<div class="ctrl-section">
        <div class="ctrl-label">📋 نتائج الاستنتاج</div>
        <div style="font-size:13px;line-height:2">${resHTML}</div>
      </div>` : ''}
      <button class="ctrl-btn reset" onclick="
        var S=simState.se1;
        S.charge=0; S.rubCount=0; S.results={}; S.activeObj=null;
        S.balloonX=.22; S.balloonY=.38;
        simStaticElec1();
      ">↺ إعادة</button>
      <div class="info-box" style="font-size:12.5px;line-height:1.9;margin-top:6px">
        🔴 البالون: شحنة موجبة ➕<br>
        🔵 الشعر: شحنة سالبة ➖<br>
        🟢 يُجذب: الورق، الخيط، البلاستيك<br>
        ⛔ لا يُجذب: الفلزات (ذهب، نحاس، فضة)
      </div>
    `);
  }
  buildControls();
  ...
  function handleDown(e) {
    const p = gp(e), w = cv.width, h = cv.height;
    const bx = S.balloonX * w, by = S.balloonY * h;
    if (Math.hypot(p.x - bx, p.y - by) < 58) {
      S.drag = true; S.ox = p.x - bx; S.oy = p.y - by; S.lastRubY = p.y;
    }
  }
  function handleMove(e) {
    if (!S.drag) return;
    const p = gp(e), w = cv.width, h = cv.height;
    const newX = Math.max(.04, Math.min(.88, (p.x - S.ox) / w));
    const newY = Math.max(.05, Math.min(.62, (p.y - S.oy) / h));
    const personX = w * .62, personY = h * .38;
    const nearHair = Math.hypot(newX * w - personX, newY * h - personY) < w * .15;
    if (nearHair && S.charge < 5) {
      S.rubCount += Math.abs(p.y - S.lastRubY) * 0.55;
      if (S.rubCount > 28) {
        S.charge = Math.min(5, S.charge + 1);
        S.rubCount = 0;
        const el = document.getElementById('seCharge');
        if (el) el.textContent = 'الشحنة: ' + '➕'.repeat(S.charge);
        try { U9Sound.ping(280 + S.charge * 60, .1, .12); } catch (_) {}
        if (S.charge >= 3) buildControls();
      }
    }
    S.lastRubY = p.y; S.balloonX = newX; S.balloonY = newY;
  }
  function handleUp() { S.drag = false; }
  ...
  cv.addEventListener('touchend',   e => { e.preventDefault(); handleUp(); },   { passive: false });
  ...
  function draw() {
    ...
    const bx = S.balloonX * w, by = S.balloonY * h;
    const bR = Math.max(36, Math.min(50, w * 0.072));
    const bo = S.drag ? 0 : Math.sin(S.t * 1.8) * 3;
    const charged = S.charge >= 3;

    // ── Person (top right area) ──
    const personX = w * .62, personY = h * .38;
    const nearHair = Math.hypot(bx - personX, by - personY) < w * .16;
    if (nearHair && S.drag) {
      const g = c.createRadialGradient(personX, personY - 28, 8, personX, personY - 28, 62);
      g.addColorStop(0, 'rgba(255,200,0,0.45)'); g.addColorStop(1, 'rgba(255,200,0,0)');
      c.fillStyle = g; c.beginPath(); c.arc(personX, personY - 28, 62, 0, Math.PI * 2); c.fill();
    }
    c.font = Math.max(68, w * .12) + 'px serif';
    c.textAlign = 'center'; c.textBaseline = 'middle';
    c.fillText('👤', personX, personY);

    // Hair strands
    for (let i = 0; i < 7; i++) {
      const ha = -Math.PI / 2 + (i - 3) * .30;
      const splay = S.charge > 0 ? 26 + i * 5 : 13;
      const lift  = S.charge > 0 ? -10 - i * 3 : 0;
      c.strokeStyle = '#6A4C2A'; c.lineWidth = 2.2;
      c.beginPath();
      c.moveTo(personX + Math.cos(ha) * 23, personY - 34);
      c.lineTo(personX + Math.cos(ha - .24) * (23 + splay), personY - 34 + lift - Math.sin(Math.abs(ha) + .2) * splay);
      c.stroke();
    }
    if (S.charge > 0) {
      const cnt = Math.min(S.charge * 2, 10);
      c.font = `bold ${13 + S.charge}px Arial`;
      for (let i = 0; i < cnt; i++) {
        const ha = -Math.PI / 2 + (i / cnt - .5) * 1.9;
        const hl = 36 + (i % 3) * 10;
        c.fillStyle = '#C0392B'; c.textAlign = 'center'; c.textBaseline = 'middle';
        c.fillText('➖', personX + Math.cos(ha - .22) * hl, personY - 36 - Math.sin(Math.abs(ha) + .1) * hl * .9);
      }
      c.font = 'bold 12px Tajawal'; c.fillStyle = '#C0392B'; c.textAlign = 'center'; c.textBaseline = 'top';
      c.fillText('شحنات سالبة ➖', personX, personY + 50);
    }

    // ── Balloon ──
    if (S.charge > 0) { c.shadowBlur = 26; c.shadowColor = 'rgba(231,76,60,.4)'; }
    c.fillStyle = S.charge > 0 ? '#E74C3C' : '#F1948A';
    c.beginPath(); c.ellipse(bx, by + bo, bR * .84, bR, 0, 0, Math.PI * 2); c.fill();
    c.shadowBlur = 0;
    c.strokeStyle = '#C0392B'; c.lineWidth = 1.8;
    c.beginPath(); c.ellipse(bx, by + bo, bR * .84, bR, 0, 0, Math.PI * 2); c.stroke();
    // String
    c.strokeStyle = '#888'; c.lineWidth = 1.4;
    c.beginPath(); c.moveTo(bx, by + bo + bR);
    c.bezierCurveTo(bx + 7, by + bo + bR + 20, bx - 4, by + bo + bR + 44, bx, by + bo + bR + 62); c.stroke();
    // Minus signs on balloon
    if (S.charge > 0) {
      c.font = `bold ${12 + S.charge * 2}px Arial`; c.fillStyle = '#7B241C';
      for (let i = 0; i < Math.min(S.charge * 2, 10); i++) {
        const a = (i / 10) * Math.PI * 2 + S.t * .35;
        c.textAlign = 'center'; c.textBaseline = 'middle';
        c.fillText('➕', bx + Math.cos(a) * bR * .66, by + bo + Math.sin(a) * bR * .80);
      }
      c.font = 'bold 12px Tajawal'; c.fillStyle = '#C0392B'; c.textAlign = 'center'; c.textBaseline = 'top';
      c.fillText('شحنات موجبة ➕', bx, by + bo + bR + 16);
    }
    c.font = `${Math.round(bR * .58)}px serif`;
    c.textAlign = 'center'; c.textBaseline = 'middle';
    c.fillText('🎈', bx, by + bo);

    // ── Objects Row at bottom ──
    const N = OBJECTS.length;
    const slotW = w / N;
    const shelfY = h * 0.72;          // top of shelf
    const shelfH = h * 0.22;          // shelf height
    const eCY    = shelfY + shelfH * 0.42;   // emoji center Y
    const emojiSz = Math.max(40, Math.min(56, w * 0.082));

    // Shelf background strip
    c.fillStyle = 'rgba(200,160,60,0.07)';
    c.fillRect(0, shelfY, w, shelfH);
    c.strokeStyle = 'rgba(200,160,60,0.25)'; c.lineWidth = 1.5;
    c.beginPath(); c.moveTo(0, shelfY); c.lineTo(w, shelfY); c.stroke();

    S.activeObj = null;

    OBJECTS.forEach(function(obj, i) {
      const cx   = slotW * i + slotW * 0.5;
      const dist = Math.hypot(bx - cx, (by + bo) - eCY);
      const pullZone = w * 0.28;
      const result   = S.results[obj.id];

      // Is balloon near this slot?
      const near = charged && !result && dist < pullZone;
      if (near) S.activeObj = obj.id;

      // NON-METALS drift toward balloon strongly; metals stay put
      var drawX = cx, drawY = eCY;
      if (near && obj.attracted) {
        const pull = Math.pow(1 - dist / pullZone, 1.5) * 52;
        const dx = bx - cx, dy = (by + bo) - eCY;
        const len = Math.hypot(dx, dy) || 1;
        drawX = cx + (dx / len) * pull;
        drawY = eCY + (dy / len) * pull;
      }

      // Record result when balloon is close enough
      // Non-metals: record as 'attracted' when they reach the balloon
      // Metals: record as 'repelled' only when user deliberately brings balloon very close (intentional tap)
      if (charged && !result) {
        if (obj.attracted && dist < bR + emojiSz * 0.55) {
          // Non-metal touches balloon → attracted
          S.results[obj.id] = 'attracted';
          try { U9Sound.ping(520, .15, .18); } catch(e){}
          buildControls();
        } else if (!obj.attracted && dist < emojiSz * 0.7) {
          // Balloon deliberately placed ON metal → repelled (requires very close approach)
          S.results[obj.id] = 'repelled';
          try { U9Sound.ping(220, .15, .18); } catch(e){}
          buildControls();
        }
      }
      ...
      var slotPad = 5;
      var rx = cx - slotW / 2 + slotPad;
      var ry = shelfY + 4;
      var rw = slotW - slotPad * 2;
      var rh = shelfH - 8;
      var bgCol = 'rgba(255,255,255,0.55)';
      if (result === 'attracted') bgCol = 'rgba(39,174,96,0.22)';
      else if (result === 'repelled') bgCol = 'rgba(192,57,43,0.18)';
      else if (near && obj.attracted) bgCol = 'rgba(39,174,96,0.12)';
      c.fillStyle = bgCol;
      c.strokeStyle = result === 'attracted' ? 'rgba(39,174,96,0.6)'
                    : result === 'repelled'  ? 'rgba(192,57,43,0.5)'
                    : (near && obj.attracted) ? 'rgba(39,174,96,0.5)'
                    : 'rgba(200,160,60,0.2)';
      c.lineWidth = result ? 2 : (near && obj.attracted) ? 2 : 1.5;
      // glow on attracted approach
      if (near && obj.attracted && !result) {
        c.save();
        c.shadowBlur = 14;
        c.shadowColor = 'rgba(39,174,96,0.55)';
      }
      c.beginPath(); c.roundRect(rx, ry, rw, rh, 10); c.fill(); c.stroke();
      if (near && obj.attracted && !result) c.restore();

      // ── Attraction arrow (non-metals only) ──
      if (near && obj.attracted && dist < pullZone * 0.95) {
        var prox = 1 - dist / pullZone;
        var adx = bx - cx, ady = (by + bo) - eCY;
        var alen = Math.hypot(adx, ady) || 1;
        var arLen = Math.min(prox * 70, 55);
        var ax2 = cx + (adx / alen) * arLen;
        var ay2 = eCY + (ady / alen) * arLen;
        var arAlpha = 0.5 + prox * 0.5;
        c.save();
        c.shadowBlur = 8; c.shadowColor = 'rgba(39,174,96,0.6)';
        c.strokeStyle = `rgba(39,174,96,${arAlpha})`; c.lineWidth = 3.5; c.lineCap = 'round';
        c.beginPath(); c.moveTo(cx, eCY); c.lineTo(ax2, ay2); c.stroke();
        var ha2 = Math.atan2(ady, adx);
        c.beginPath(); c.moveTo(ax2, ay2);
        c.lineTo(ax2 - Math.cos(ha2 - 0.42) * 14, ay2 - Math.sin(ha2 - 0.42) * 14);
        c.lineTo(ax2 - Math.cos(ha2 + 0.42) * 14, ay2 - Math.sin(ha2 + 0.42) * 14);
        c.closePath(); c.fillStyle = `rgba(39,174,96,${arAlpha})`; c.fill();
        c.restore();
      }

      // ── White circle behind emoji ──
      c.save();
      c.beginPath();
      c.arc(drawX, drawY, emojiSz * 0.68, 0, Math.PI * 2);
      c.fillStyle = 'rgba(255,255,255,0.92)';
      c.shadowBlur = (near && obj.attracted) ? 14 : 0;
      c.shadowColor = 'rgba(142,68,173,0.5)';
      c.fill();
      c.restore();

      // ── Emoji ──
      c.font = emojiSz + 'px serif';
      c.textAlign = 'center'; c.textBaseline = 'middle';
      c.fillText(obj.emoji, drawX, drawY);

      // ── Name ──
      var nameY = shelfY + shelfH * 0.72;
      c.font = 'bold ' + Math.max(11, Math.min(14, w * 0.023)) + 'px Tajawal';
      c.fillStyle = result === 'attracted' ? '#27AE60'
                  : result === 'repelled'  ? '#C0392B'
                  : 'rgba(80,55,15,0.9)';
      c.textAlign = 'center'; c.textBaseline = 'middle';
      c.fillText(obj.name, cx, nameY);

      // ── Result badge ──
      if (result) {
        var badgeY = shelfY + shelfH * 0.90;
        c.font = 'bold ' + Math.max(10, Math.min(12, w * 0.020)) + 'px Tajawal';
        c.fillStyle = result === 'attracted' ? '#27AE60' : '#C0392B';
        c.textAlign = 'center'; c.textBaseline = 'middle';
        c.fillText(result === 'attracted' ? '✅ يُجذب' : '❌ لا يُجذب', cx, badgeY);
      }
    });

    // ── Hint ──
    const allTested = OBJECTS.every(o => S.results[o.id]);
    const hint = allTested
      ? '🎉 أحسنت! راجع نتائج الاستنتاج في اللوحة الجانبية'
      : S.charge === 0
        ? '✋ اسحب البالون واحتكه بشعر الشخص لشحنه'
        : S.charge < 3
          ? `⚡ استمر في الاحتكاك... (${S.charge}/5)`
          : '🎈 البالون مشحون! اسحبه نحو كل جسم في الأسفل';
    c.font = `bold ${Math.min(13, w * .026)}px Tajawal`;
    c.fillStyle = 'rgba(120,80,20,.65)'; c.textAlign = 'center'; c.textBaseline = 'bottom';
    c.fillText(hint, w / 2, h * .97);
...
function simStaticElec2() {
  ...
  if(!simState.se2) simState.se2={q1:'pos',q2:'neg',balls:[{x:.3,y:.45,vx:0},{x:.7,y:.45,vx:0}],drag:-1,ox:0,oy:0,t:0,animating:false};
  ...
  function rebuild(){
    controls(`
      <div class="ctrl-section">
        ...
        <button class="ctrl-btn${S.q1==='pos'?' active':''}" onclick="simState.se2.q1='pos';simState.se2.animating=true;window._se2rebuild()" style="${S.q1==='pos'?'background:#C0392B;color:#fff':''}">➕ موجبة</button>
        <button class="ctrl-btn${S.q1==='neg'?' active':''}" onclick="simState.se2.q1='neg';simState.se2.animating=true;window._se2rebuild()" style="${S.q1==='neg'?'background:#2980B9;color:#fff':''}">➖ سالبة</button>
      </div>
      <div class="ctrl-section">
        <div class="ctrl-label">⚡ الكرة اليسرى</div>
        <button class="ctrl-btn${S.q2==='pos'?' active':''}" onclick="simState.se2.q2='pos';simState.se2.animating=true;window._se2rebuild()" style="${S.q2==='pos'?'background:#C0392B;color:#fff':''}">➕ موجبة</button>
        <button class="ctrl-btn${S.q2==='neg'?' active':''}" onclick="simState.se2.q2='neg';simState.se2.animating=true;window._se2rebuild()" style="${S.q2==='neg'?'background:#2980B9;color:#fff':''}">➖ سالبة</button>
      </div>
      <button class="ctrl-btn reset" onclick="simState.se2.balls[0].x=.3;simState.se2.balls[1].x=.7;simState.se2.balls[0].vx=0;simState.se2.balls[1].vx=0;simState.se2.animating=true">↺ إعادة الوضع</button>
      <div class="info-box" style="font-size:13px;line-height:1.9">
        ➕ + ➖ = <strong style="color:#27AE60">تجاذب ❤️ تقترب</strong><br>
        ➕ + ➕ = <strong style="color:#E74C3C">تنافر 💥 تتباعد</strong><br>
        ➖ + ➖ = <strong style="color:#E74C3C">تنافر 💥 تتباعد</strong>
      </div>`);
  }
  ...
  function gp(e){const r=cv.getBoundingClientRect(),sc=cv.width/r.width,s=e.touches&&e.touches[0]||e.changedTouches&&e.changedTouches[0]||e;return{x:(s.clientX-r.left)*sc,y:(s.clientY-r.top)*sc};}
  function down(e){e.preventDefault();const p=gp(e),w=cv.width,h=cv.height;S.balls.forEach((b,i)=>{if(Math.hypot(p.x-b.x*w,p.y-b.y*h)<34){S.drag=i;S.ox=p.x-b.x*w;S.oy=p.y-b.y*h;b.vx=0;}});}
  ...
  function up(){if(S.drag>=0){S.animating=true;}S.drag=-1;}
  cv.addEventListener('mousedown',down);cv.addEventListener('mousemove',move);cv.addEventListener('mouseup',up);
  cv.addEventListener('touchstart',down,{passive:false});cv.addEventListener('touchmove',move,{passive:false});cv.addEventListener('touchend',up);
  ...
  function draw(){
    ...
    if(S.drag<0){
      ...
      const dx=(b1.x-b0.x)*w;
      const dist=Math.abs(dx);
      const force=Math.min(0.0015, 2200/(dist*dist+100));

      if(attract){
        // move toward each other
        if(dist>50){ // min approach distance (touching)
          b0.vx+=(dx>0?force:-force);
          b1.vx+=-(dx>0?force:-force);
        } else {
          // touching — slight bounce and stop
          b0.vx*=0.6; b1.vx*=0.6;
          b0.x=.5-26/w; b1.x=.5+26/w;
        }
      } else {
        // repel — move apart
        b0.vx-=(dx>0?force:-force);
      ...
      b0.vx*=0.92; b1.vx*=0.92; // friction
      b0.x=Math.max(.06,Math.min(.5,b0.x+b0.vx));
      b1.x=Math.max(.5,Math.min(.94,b1.x+b1.vx));
    }
    ...
    // Connection line
    if(dist<w*.7){
      const force=Math.max(0,1-dist/(w*.65));
      const col=attract?'#27AE60':'#E74C3C';
      c.strokeStyle=col+Math.round(force*180).toString(16).padStart(2,'0');
      c.lineWidth=force*5+1; c.setLineDash([6,4]);
      c.beginPath(); c.moveTo(x0,y0); c.lineTo(x1,y1); c.stroke(); c.setLineDash([]);

      // Status label
      const midX=(x0+x1)/2, midY=(y0+y1)/2;
      if(attract&&dist<70){
        // Touching visual effect
        c.fillStyle='rgba(39,174,96,0.15)'; c.beginPath(); c.arc(midX,midY,40,0,Math.PI*2); c.fill();
        c.font='bold 16px Tajawal'; c.fillStyle='#27AE60'; c.textAlign='center'; c.textBaseline='middle';
        c.fillText('❤️ تلامس',midX,midY-36);
      } else if(force>.06){
        c.font=`bold ${12+force*7|0}px Tajawal`; c.fillStyle=col; c.textAlign='center'; c.textBaseline='middle';
        c.fillText(attract?'❤️ تجاذب':'💥 تنافر',midX,midY-36);
      }

      // Movement arrows
      if(attract&&dist>80){
        const ax=(x1-x0)/dist*18, ay=(y1-y0)/dist*18;
        c.strokeStyle='#27AE60'; c.lineWidth=2.5; c.fillStyle='#27AE60';
        // Arrow from b0 toward b1
        c.beginPath(); c.moveTo(x0+30,y0); c.lineTo(x0+30+ax*1.5,y0+ay*1.5); c.stroke();
        c.beginPath(); c.moveTo(x1-30,y1); c.lineTo(x1-30-ax*1.5,y1-ay*1.5); c.stroke();
      } else if(!attract){
        const repD=Math.min(40,dist*.25);
        c.strokeStyle='#E74C3C'; c.lineWidth=2.5;
        c.beginPath(); c.moveTo(x0+25,y0); c.lineTo(x0+25-repD*.6,y0); c.stroke();
        c.beginPath(); c.moveTo(x1-25,y0); c.lineTo(x1-25+repD*.6,y0); c.stroke();
      }
    }
    ...
    const hint=attract?'❤️ أقطاب مختلفة → تتجاذب وتلتصق':'💥 أقطاب متشابهة → تتنافر وتتباعد';
    U9.txt(c,hint,w/2,h-10,'rgba(80,100,140,.5)',12,true);
    animFrame=requestAnimationFrame(()=>{try{draw();}catch(e){}});
  }
  draw();
}
...
function simCircuit8Series() {
  ...
  function draw(){
    ...
    U9.txt(c,`الدائرة على التوالي · I = ${I} A`,w/2,h*.12,'#6C3483',14,true);
    if(!allOn) U9.txt(c,'⚠️ قطع التوالي — الكل انطفأ!',w/2,h*.88,'#E74C3C',13,true);
    animFrame=requestAnimationFrame(()=>{try{draw();}catch(e){}});
  }
  draw();
}
...
function simCircuit8Parallel() {
  ...
  function draw(){
    ...
    U9.txt(c,`الدائرة على التوازي · ${onCnt} مصابيح تضيء`,w/2,h*.12,'#1E8449',14,true);
...
// ══════════════════════════════════════════════════════════════
// 11-9 · TAB 3 — استقصاء دائرة التوالي
// ══════════════════════════════════════════════════════════════
function simCircuit8Mystery() {
  cancelAnimationFrame(animFrame);
  const cv = document.getElementById('simCanvas');
  const c  = cv.getContext('2d');

  if (!simState.c8m) simState.c8m = {
    voltage: 6, ammeterPos: 0, bulbCount: 1, revealed: false, t: 0, r: 15
  };
  const S = simState.c8m;

  function renderControls() {
    const bulbLabels = ['مصباح ١','مصباح ١، مصباح ٢','مصباح ١، مصباح ٢، مصباح ٣'];
    controls(`
      <div class="ctrl-section">
        <div class="ctrl-label">🔋 الجهد: <span class="ctrl-val">${S.voltage}</span> V</div>
        <input type="range" min="3" max="12" step="3" value="${S.voltage}"
          oninput="simState.c8m.voltage=+this.value;simState.c8m.revealed=false;simCircuit8Mystery()">
      </div>
      <div class="ctrl-section">
        <div class="ctrl-label">💡 عدد المصابيح: <span class="ctrl-val">${S.bulbCount}</span></div>
        <input type="range" min="1" max="3" step="1" value="${S.bulbCount}"
          oninput="simState.c8m.bulbCount=+this.value;simState.c8m.revealed=false;simCircuit8Mystery()">
        <div style="font-size:11px;color:var(--muted);margin-top:4px;text-align:center">${bulbLabels[S.bulbCount-1]}</div>
      </div>
      <div class="ctrl-section">
        <div class="ctrl-label">📍 موضع الأميتر</div>
        ${[0,1,2].map(i=>`
          <button class="ctrl-btn${S.ammeterPos===i?' play':''}"
            onclick="simState.c8m.ammeterPos=${i};simState.c8m.revealed=false;simCircuit8Mystery()">
            ${['① قبل مصباح ١','② بين مصباح ١ و ٢','③ بعد آخر مصباح'][i]}
          </button>`).join('')}
      </div>
      <div class="info-box" style="font-size:12px;line-height:1.9">
        🔍 <strong>مهمتك:</strong><br>
        حرّك الأميتر للمواضع الثلاثة<br>
        وغيّر عدد المصابيح<br>
        <em style="color:var(--muted)">ماذا تلاحظ؟</em>
      </div>
      <button class="ctrl-btn play" onclick="simState.c8m.revealed=!simState.c8m.revealed;simCircuit8Mystery()"
        style="margin-top:8px;width:100%">
        ${S.revealed ? '🙈 أخفِ التفسير' : '💡 اكشف الحقيقة!'}
      </button>
    `);
  }
  ...
  function draw() {
    if (currentSim !== 'circuit8' || currentTab !== 2) return;
    S.t += 0.035;
    const w = cv.width, h = cv.height;
    c.clearRect(0, 0, w, h);

    // ── خلفية فاتحة ──
    c.fillStyle = '#EEE9E2'; c.fillRect(0, 0, w, h);

    // ── حسابات ──
    const R_total = S.r * S.bulbCount;
    const I = +(S.voltage / R_total).toFixed(2);
    // إضاءة واضحة: مصباح واحد = MAX، كل مصباح إضافي يقلل بشكل ملحوظ
    const I_single = S.voltage / S.r;
    const brightness = I / I_single; // 1.0 لمصباح واحد، 0.5 لمصباحين، 0.33 لثلاثة
    ...
    const wireClr = '#1A8FA8';

    // ── رسم الأسلاك ──
    c.strokeStyle = wireClr; c.lineWidth = 4.5;
    c.lineCap = 'round';
    c.beginPath(); c.moveTo(left, top);  c.lineTo(right, top);  c.stroke();
    c.beginPath(); c.moveTo(left, bot);  c.lineTo(right, bot);  c.stroke();
    c.beginPath(); c.moveTo(right, top); c.lineTo(right, bot);  c.stroke();
    c.beginPath(); c.moveTo(left,  top); c.lineTo(left,  bot);  c.stroke();

    // نقاط زوايا
    [[left,top],[right,top],[right,bot],[left,bot]].forEach(([dx,dy])=>{
      c.fillStyle='#27AE60'; c.beginPath(); c.arc(dx,dy,7,0,Math.PI*2); c.fill();
    });

    // ── إلكترونات ──
    for (let i = 0; i < 9; i++) {
      const frac = ((S.t * 0.22 + i/9) % 1);
      const topW=right-left, sideH=bot-top, perim=2*(topW+sideH);
      const dist=frac*perim;
      let ex,ey;
      if      (dist < topW)           { ex=left+dist;                ey=top; }
      else if (dist < topW+sideH)     { ex=right;                    ey=top+(dist-topW); }
      else if (dist < 2*topW+sideH)   { ex=right-(dist-topW-sideH); ey=bot; }
      else                            { ex=left; ey=bot-(dist-2*topW-sideH); }
      const alpha=0.55+0.35*Math.sin(S.t*4+i);
      c.fillStyle=`rgba(26,143,168,${alpha})`;
      c.beginPath(); c.arc(ex,ey,5,0,Math.PI*2); c.fill();
    }

    // ── بطارية ──
    function drawBattery(cx,cy) {
      c.fillStyle='#F5F0E8'; c.strokeStyle='#999'; c.lineWidth=1.5;
      c.beginPath(); c.roundRect(cx-16,cy-38,32,76,5); c.fill(); c.stroke();
      [['#E74C3C',cy-28],['#F39C12',cy-10],['#27AE60',cy+8]].forEach(([col,y])=>{
        c.fillStyle=col; c.fillRect(cx-11,y,22,12);
      });
      c.textAlign='center'; c.textBaseline='middle';
      c.font='bold 15px sans-serif';
      c.fillStyle='#E74C3C'; c.fillText('+',cx,cy-52);
      c.fillStyle='#3498DB';  c.fillText('−',cx,cy+54);
      c.font='bold 13px Tajawal'; c.fillStyle='#2C3A4A'; c.textBaseline='alphabetic';
      c.fillText(S.voltage+'V',cx,cy+72);
    }
    drawBattery(left, midY);

    // ── توزيع المصابيح ──
    // المصباح الأخير دائماً أسفل · الباقون أعلى
    const topCount = S.bulbCount - 1;
    const botBulbX = (left + right) / 2;

    // المصابيح العلوية: الثلث الأيمن من السلك العلوي
    const topAreaStart = left + (right-left)*0.35;
    const topAreaEnd   = right - (right-left)*0.05;
    const topBulbX = [];
    for (let i=0; i<topCount; i++) {
      const x = topCount===1
        ? (topAreaStart+topAreaEnd)/2
        : topAreaStart + (topAreaEnd-topAreaStart)*i/(topCount-1);
      topBulbX.push(x);
    }

    // ── دالة رسم المصباح كرمز كهربائي ──
    function drawBulb(bx, by, nameLabel, brt, side) {
      // side: 'top' → فوق السلك · 'bot' → تحت السلك
      const R = 22;
      const isTop = side === 'top';
      const cy2 = isTop ? by - R - 2 : by + R + 2;

      // فجوة بسيطة في السلك
      c.strokeStyle='#EEE9E2'; c.lineWidth=6;
      c.beginPath(); c.moveTo(bx-R+4,by); c.lineTo(bx+R-4,by); c.stroke();
      c.strokeStyle=wireClr; c.lineWidth=4.5;

      // هالة إضاءة (تتغير بشكل واضح)
      const glowSize = R * (1.5 + brt * 2.5);
      const glowAlpha = brt * 0.7;
      if (brt > 0.05) {
        const glow = c.createRadialGradient(bx,cy2,2,bx,cy2,glowSize);
        glow.addColorStop(0,`rgba(255,220,50,${glowAlpha})`);
        glow.addColorStop(0.5,`rgba(255,180,0,${glowAlpha*0.4})`);
        glow.addColorStop(1,`rgba(255,150,0,0)`);
        c.fillStyle=glow; c.beginPath(); c.arc(bx,cy2,glowSize,0,Math.PI*2); c.fill();
      }

      // جسم المصباح
      const lum = Math.floor(brt * 255);
      const bulbFill = brt > 0.1
        ? `rgb(255,${Math.floor(130+brt*110)},${Math.floor(brt*40)})`
        : '#DCDCDC';
      c.fillStyle=bulbFill;
      c.strokeStyle=brt>0.2?`rgba(180,100,0,0.8)`:'#AAA';
      c.lineWidth=2.5;
      c.beginPath(); c.arc(bx,cy2,R,0,Math.PI*2); c.fill(); c.stroke();

      // خيط داخلي
      c.strokeStyle = brt>0.2 ? `rgba(255,255,255,${0.5+brt*0.5})` : '#BBB';
      c.lineWidth=2.2;
      c.beginPath();
      c.moveTo(bx-7,cy2+8); c.quadraticCurveTo(bx,cy2-2,bx+7,cy2+8);
      c.stroke();
      c.beginPath();
      c.moveTo(bx-4,cy2+8); c.lineTo(bx-4,cy2+14);
      c.moveTo(bx+4,cy2+8); c.lineTo(bx+4,cy2+14);
      c.stroke();

      // وصلة بالسلك
      c.strokeStyle=wireClr; c.lineWidth=3;
      if (isTop) {
        c.beginPath(); c.moveTo(bx,cy2+R); c.lineTo(bx,by); c.stroke();
      } else {
        c.beginPath(); c.moveTo(bx,cy2-R); c.lineTo(bx,by); c.stroke();
      }

      // تسمية + مقاومة
      c.font='bold 13px Tajawal'; c.fillStyle='#1E2D3D'; c.textAlign='center';
      if (isTop) {
        c.textBaseline='top';
        c.fillText(nameLabel, bx, cy2+R+8);
        c.font='12px Tajawal'; c.fillStyle='#5A6A7A';
        c.fillText(S.r+'Ω', bx, cy2+R+24);
      } else {
        c.textBaseline='bottom';
        c.fillText(nameLabel, bx, cy2-R-8);
        c.font='12px Tajawal'; c.fillStyle='#5A6A7A';
        c.fillText(S.r+'Ω', bx, cy2-R-22);
      }
      c.textBaseline='alphabetic';
    }

    // رسم المصابيح
    const bulbNames=['مصباح ١','مصباح ٢','مصباح ٣'];
    for (let i=0; i<topCount; i++) drawBulb(topBulbX[i], top, bulbNames[i], brightness, 'top');
    drawBulb(botBulbX, bot, bulbNames[S.bulbCount-1], brightness, 'bot');

    // ── الأميتر (داخل الدائرة على السلك) ──
    // موضع 0 → السلك العلوي يسار المصابيح
    // موضع 1 → السلك العلوي بين مصباح ١ و ٢ (أو الأسفل إذا bulbCount=1)
    // موضع 2 → السلك الأسفل يمين المصباح الأخير
    let aX, aY;
    if (S.ammeterPos === 0) {
      aX = left + (right-left)*0.20; aY = top;
    } else if (S.ammeterPos === 1) {
      if (topCount >= 2 && topBulbX.length >= 2) {
        aX = (topBulbX[0]+topBulbX[1])/2; aY = top;
      } else if (topCount === 1) {
        aX = (topBulbX[0] + right)*0.5; aY = top;
      } else {
        aX = botBulbX - (right-left)*0.15; aY = bot;
      }
    } else {
      aX = botBulbX + (right-left)*0.18; aY = bot;
    }
    const ammeterLabels=['قبل مصباح ١','بين مصباح ١ و ٢','بعد آخر مصباح'];

    const aR = 22;
    // فجوة في السلك
    c.strokeStyle='#EEE9E2'; c.lineWidth=6;
    c.beginPath(); c.moveTo(aX-aR+4,aY); c.lineTo(aX+aR-4,aY); c.stroke();

    // نبضة
    const pulse=0.25+0.25*Math.sin(S.t*3.5);
    c.fillStyle=`rgba(26,95,122,${pulse})`;
    c.beginPath(); c.arc(aX,aY,aR*1.6,0,Math.PI*2); c.fill();

    // دائرة الأميتر (مركزها على السلك مباشرة)
    c.fillStyle='#FFFFFF'; c.strokeStyle='#1A5F7A'; c.lineWidth=3;
    c.beginPath(); c.arc(aX,aY,aR,0,Math.PI*2); c.fill(); c.stroke();
    c.font='bold 18px sans-serif'; c.fillStyle='#1A5F7A';
    c.textAlign='center'; c.textBaseline='middle';
    c.fillText('A',aX,aY);
    c.textBaseline='alphabetic';

    // بطاقة قراءة التيار (فوق أو تحت حسب الموضع)
    const cardW=100, cardH=42;
    const cardX=aX-cardW/2;
    const cardY = (aY===top) ? aY-aR-cardH-10 : aY+aR+10;
    c.fillStyle='#1A5F7A'; c.strokeStyle='#4EC9F0'; c.lineWidth=2;
    c.beginPath(); c.roundRect(cardX,cardY,cardW,cardH,9); c.fill(); c.stroke();
    c.font='bold 16px Tajawal'; c.fillStyle='#FFFFFF';
    c.textAlign='center'; c.textBaseline='middle';
    c.fillText(I+' A', aX, cardY+14);
    c.font='11px Tajawal'; c.fillStyle='#AEE8F8';
    c.fillText(ammeterLabels[S.ammeterPos], aX, cardY+30);
    c.textBaseline='alphabetic';

    // ── عنوان ──
    c.font='bold 17px Tajawal'; c.fillStyle='#1E2D3D'; c.textAlign='center';
    c.fillText('⚡ استقصاء دائرة التوالي', w/2, h*0.09);

    // ── معادلة أسفل ──
    const rStr=Array(S.bulbCount).fill(S.r).join('+');
    c.font='13px Tajawal'; c.fillStyle='#B07000';
    c.fillText(`I = ${S.voltage}V ÷ (${rStr})Ω = ${I} A`, w/2, h*0.95);

    // ── كشف الحقيقة ──
    if (S.revealed) {
      const bx=w*0.5, by=h*0.52;
      c.fillStyle='rgba(0,0,0,0.06)';
      c.beginPath(); c.roundRect(bx-210,by-76+6,420,170,14); c.fill();
      c.fillStyle='#FFFEF7'; c.strokeStyle='#27AE60'; c.lineWidth=2.5;
      c.beginPath(); c.roundRect(bx-210,by-76,420,170,14); c.fill(); c.stroke();
      c.textAlign='center'; c.textBaseline='middle';
      c.font='bold 14px Tajawal'; c.fillStyle='#1E6B3C';
      c.fillText('💡 الملاحظة الأولى: التيار لا يُستهلك!', bx, by-52);
      c.font='13px Tajawal'; c.fillStyle='#2C3A4A';
      c.fillText('الأميتر يقرأ نفس القيمة في أي موضع على الدائرة', bx, by-30);
      c.fillText('لأن دائرة التوالي مسار واحد فقط.', bx, by-10);
      c.font='bold 14px Tajawal'; c.fillStyle='#B07000';
      c.fillText('💡 الملاحظة الثانية: زيادة المصابيح تقلل الإضاءة!', bx, by+18);
      c.font='13px Tajawal'; c.fillStyle='#2C3A4A';
      c.fillText(`${S.bulbCount} مصباح → R = ${R_total}Ω → I = ${I}A`, bx, by+42);
      c.fillText('كلما زاد عدد المصابيح، زادت المقاومة وقلّ التيار', bx, by+66);
      c.textBaseline='alphabetic';
    }
...
function simParallel12b() {
  ...
  controls(`
    ...
    <div class="info-box" style="font-size:12px;line-height:1.9">
      على التوالي: تيار واحد عبر الكل<br>
      على التوازي: كل فرع له تياره<br>
  ...
  function draw(){
    ...
    U9.txt(c,'على التوالي',lMid,top-18,'#8E44AD',13,true);
    ...
    U9.txt(c,'على التوازي',rMid,top-18,'#27AE60',13,true);
