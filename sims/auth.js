...
// ── إظهار الـ overlay فوراً لمنع ظهور المحتوى قبل التحقق ──
(function() {
  var overlay = document.getElementById('auth-overlay');
  if (overlay) overlay.classList.add('visible');
  // إخفاء landing وapp حتى يكتمل الفحص
...
function _lockPlatform(reason) {
  ...
  if (app) {
    if (landing) {
      // index.html: keep app hidden, show landing
      app.style.display = 'none';
      app.classList.remove('open');
    } else {
      // grade pages: show app
...
function _unlockPlatform() {
  ...
  if (gradePicker) gradePicker.style.display = 'none';
  var app = document.getElementById('app');
  if (app) {
    // index.html has a landing element → keep app hidden, show landing
    // grade pages have no landing → show app directly
...
// ── فحص الجلسة تلقائياً عند تحميل الصفحة ──
document.addEventListener('DOMContentLoaded', function() {
  _checkSubscription();
});
